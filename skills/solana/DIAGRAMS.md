...
- **The cluster curve is the positive (top) half of the beach-ball-shaped Ed25519 curve: it arcs UP.** Canonical path: `M 40,96 Q 320,12 600,96` on a width-640 canvas — it rises from its ends to an apex at `(320,54)` and descends again; the control point sits **above** both endpoints. A curve that droops in the middle is wrong, always. The cluster label sits above the apex (e.g. `x="320" y="30"`, white-haloed).
- **Every on-curve address dot — each person, each mint, the program — sits ON the curve.** For the canonical path, a dot at `cx` sits at `t = (cx − 40)/560`, `cy = 96·((1−t)² + t²) + 24·t·(1−t)`. The dot's `PUBLIC KEY` label goes in the gap between the dot and its box below (or above the dot on the outer slopes), white-haloed.
- **Everything else hangs below the curve, on a fixed box row below the curve's ends.** Wallet boxes and the program box all start at the same top (`y="104"` for the canonical path — below the curve's end height of 96), so nothing crosses the descending shoulders. **No line, arrow, or box ever crosses the curve.** The open pockets above the descending shoulders may hold a mint's small box (its dot on the curve beneath it), nothing else. The program's dot sits on the slope roughly centered above the program box.
