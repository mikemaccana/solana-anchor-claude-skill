...
- **Give the answer for each case before the principle behind them.** A reader arrives with a job, not with a desire to reason from first principles. Lead with the cases and what to do in each, including what goes wrong with each choice ("for a rate quoted per year use timestamps, but it charges straight through a halt"), then state the principle underneath in a line. A section that only states the principle has handed the reader homework: "choose by asking who benefits from the error" is the reason for the table, not a substitute for it. A chapter opener fails the same way: "Solana has two clocks, and which one a program should reach for follows from which of the two errors is bigger" hands the reader a method and no answers, where naming the jobs does the work - a price freshness window counts slots, a week promised to a contributor runs on the wall clock, and who queued first uses a counter of its own.
